...
Copy-Item "E:\sources\projects\fivem-wasm\target\wasm32-wasi\release\basic_client.wasm" -Destination "E:\sources\c\fivem-fork\code\bin\server\windows\debug\server-data\resources\wasm\" -Force
Copy-Item "E:\sources\projects\fivem-wasm\target\wasm32-wasi\release\basic_server.wasm" -Destination "E:\sources\c\fivem-fork\code\bin\server\windows\debug\server-data\resources\wasm\" -Force
...
if ($build -eq "build") {
    ...
    Invoke-Expression "& tools\ci\nuget.exe restore build\server\windows\CitizenMP.sln"
    ...
    ./MSbuild.exe /p:configuration=Debug /m E:\sources\c\fivem-fork\code\build\server\windows\CitizenMP.sln
    # ./MSbuild.exe "E:\sources\c\fivem-fork\code\build\server\windows\CitizenMP.sln" /t:Build /p:Configuration=Debug /p:Platform=x64 -m
    # ./MSbuild.exe "E:\sources\c\fivem-fork\code\build\five\CitizenMP.sln" /t:Build /p:Configuration=Release -m

    Pop-Location -StackName FiveMWasm;
    Pop-Location -StackName FiveMWasm;

    Copy-Item "E:\sources\projects\fivem-wasm\utils\components.json" -Destination "E:\sources\c\fivem-fork\code\bin\server\windows\debug\" -Force;
}
